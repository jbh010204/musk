...
test('export and import planner data restores timebox and category', async ({ page }) => {
  ...
  const createdBox = page.locator('[title="백업테스트일정"]:visible').first()
  ...
  await expect(page.locator('[title="백업테스트일정"]:visible').first()).toHaveCount(0)
  ...
  await expect(page.locator('[title="백업테스트일정"]:visible').first()).toContainText('#백업카테고리')
})
