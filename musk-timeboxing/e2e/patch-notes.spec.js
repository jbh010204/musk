...
test('floating action menu opens patch notes modal', async ({ page }) => {
  ...
  await expect(page.getByText('v0.12.8').first()).toBeVisible()
  await expect(page.getByText('v0.6.0').first()).toBeVisible()

  const latestToggle = page.getByTestId('patch-note-toggle-v0.12.8')
  const latestDetail = page.getByTestId('patch-note-detail-v0.12.8')
