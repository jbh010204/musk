...
- `주간`: 현재 주간의 일정 수, 완료율, 계획 시간, 일정 미리보기를 카드로 확인
  ...
  - 주간 상단 `Deadline Strip`은 이번 주 안에 마감되는 카드의 deadline을 `D-n / 오늘 / 지남` 상태로 먼저 보여주고, 칩 클릭 시 해당 날짜로 바로 이동합니다.
  - strip의 `완료` 버튼으로 열린 deadline을 즉시 닫을 수 있습니다.
...
- `월간`: 월 전체를 캘린더 그리드로 훑고 날짜 클릭으로 상세 시트를 연 뒤, 필요할 때만 일간 보기로 이동
  ...
  - 월간 상단 `Deadline Strip`은 이달 마감 항목을 urgency 순으로 노출하고, 칩 클릭 시 월간 상세 시트에서 해당 날짜를 먼저 선택합니다.
...
- 데드라인은 timeBox와 별개인 상위 압박 레이어입니다.
- source of truth:
  - `title`
  - `dueDate`
  - `linkedTaskId | null`
  ...
  - `note`
  - `completedAt | null`
- `D-n`, `오늘`, `지남(overdue)`은 저장하지 않고 현재 날짜 기준으로 파생합니다.
- 활성 deadline은 기본적으로 `task 1개당 1개`만 유지합니다. 카드 편집 모달에서 마감일을 저장하면 기존 활성 deadline을 수정하는 방식으로 동작합니다.
- 캔버스 카드 표면에는 linked deadline badge가 직접 노출됩니다. `D-n`, `오늘`, `지남`, `완료` 상태가 카드 위에서 바로 보입니다.
- 카드 우측의 작은 토글로 deadline을 완료하거나 다시 열 수 있고, 주간/월간 strip의 `완료` 버튼도 같은 상태를 공유합니다.
- deadline은 주간/월간 상단 strip에서 먼저 스캔하고, 실제 실행 시간 배치는 계속 일간 timeBox가 담당합니다.
- 현재 단계에서는 deadline 완료 전용 UI는 없고, 카드 편집 모달에서 생성/수정/삭제만 지원합니다.
...
이 프로젝트에서는 새로운 프론트 기능이나 상태가 있는 리팩토링을 바로 구현하지 않고, 먼저 `/fqa`를 사용해 기능의 목적, 행복 경로, 예외, 가드, 불변조건을 짧게 구조화합니다.

권장 규칙:
- 비사소한 프론트 기능 구현 전에는 왠만하면 `/fqa <기능명>`부터 시작
- drag/drop, 모드 전환, persistence, route guard, multi-step UX, 카테고리/타임라인 동기화는 `/fqa`를 기본값으로 사용
- copy edit, 단순 spacing, one-line 스타일 수정처럼 흐름 영향이 없는 작업만 예외
