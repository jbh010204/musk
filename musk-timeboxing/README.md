...
- `보드`: Excalidraw처럼 보이는 카테고리 스택 보드에서 브레인 덤프 카드를 시각적으로 정리
  - 카테고리 원형 노드 아래 stack lane으로 카드가 쌓이며, 카드를 선택한 뒤 노드를 클릭하거나 드래그로 카테고리 이동 가능
  - 카드 생성 시 예상 길이와 메모를 함께 넣고, 실제 시간 배치는 이후 `편성기` 단계에서 처리
- `캔버스`: tldraw 기반 무한 캔버스 + custom shape + inspector
  - 날짜별 `boardCanvas` snapshot을 저장하고, 기존 보드 카드를 자유 배치형 scene으로 자동 투영
  - 현재 단계는 기본 UI를 숨기고 planner toolbar + 저장/복원 + 자동 배치만 먼저 연결한 상태
- `편성기`: 보드 카드를 시간표로 보내기 위한 단계
...
- Planning Board는 이 리스트를 대체할 차세대 인터페이스이며, 현재는 리스트 fallback과 함께 병행됩니다.
...
- tldraw 캔버스 아키텍처: `docs/TLDRAW_CANVAS_ARCHITECTURE.md`
